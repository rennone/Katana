...
public partial class ActorMotor : MonoBehaviour
{
    ...
    bool useFixedUpdate = false;    //fixedUpdateでinput入力を受け取るのはよくないので切る
    ...
    CollisionFlags Move(Vector3 move)
    {
        // それぞれの軸のConstraint設定
    ...
    void lockZAxis(CollisionFlags collisionFlags, Vector3 oldPosition, ref Vector3 position)
    {
    ...
    void GroundedUpdate(float pushDownOffset)
    {
        ...
        if (movingPlatform.enabled)
        {
            if (movingPlatform.activePlatform != null)
            {
                if (!movingPlatform.hasNewPlatform)
                {
                    ...
                    Vector3 distance = movingPlatform.activePlatform.localToWorldMatrix.MultiplyPoint3x4(movingPlatform.activeLocalPoint)
    ...
    float MaxSpeedInDirection(Vector3 desiredMovementDirection)
    {
        ...
        {
            float zAsixMaxSpeed = desiredMovementDirection.z > 0 ? movement.MaxForwardSpeed: movement.MaxBackwardsSpeed;
